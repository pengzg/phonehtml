tempApp.controller('ctr_orderProduct', function($scope, $rootScope, http, messageFactory, $state, $stateParams, EzConfirm,dateUtil,$rootScope) {

  $scope.pager = {page:1,rows:'20',sort:'ogm_order_time',order:'desc',om_order_type:'2',pageList:['10','20','30'],om_shopid:$rootScope.USER.shopId};
  $scope.searchParam = {};
  ...
  if ($scope.memberType == '7') {
	  $scope.pager.om_shopid = "";
  }
  $('.right-pop').hide();
  ...
  var queryList = function(){
	  ...
	  $scope.searchParam.om_businesstype = 3;
	  ...
	  var url = '/admin/ordergoods/orderGoodsMainControl/dataGrid.action';
  ...
  /**
	...
	$scope.goOrderDetail = function(id,code){
		var url = "";
		url = $state.href('index.order.orderProductDetail',{'om_id':id,'om_order_code':code});
	...
	$scope.outNumChange = function(x){
		...
		if(x.od_product_num<x.od_delivery_num+x.outNum){
			x.outNum = x.od_product_num - x.od_delivery_num;
		}
		if(x.pss_stocknum<=0){
			x.outNum = 0;
		}
		else if(x.pss_stocknum<=x.outNum){
			x.outNum = x.pss_stocknum;
		}
	}
	...
	$scope.queryDeliveryman = function(){
		...
		$scope.baseWorkSearchParam.mbw_isdelivery = 'Y';
		var url = '/admin/member/memberBaseWorkControl/dataGrid.action';
		http.post(url,$.extend({"mbw_shopid":$rootScope.USER.shopId},$scope.baseWorkPager,$scope.baseWorkSearchParam),success,error);
	}
	/**
	 * 选择配送人员
	 */
	 ...
	 * 退款水票订单
	 */
	$scope.refundOrderSS = function(obj){
		var success = function(result){
			messageFactory.closeLoading();
			messageFactory.showMessage('success',result.desc);
			queryList();
		...
		var data ={'om_id':obj.om_id}
		var url = "/admin/order/orderRefundControl/refundOrderSS.action";
