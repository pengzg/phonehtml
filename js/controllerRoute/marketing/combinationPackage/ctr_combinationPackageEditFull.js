tempApp.controller('ctr_combinationPackageEditFull', function($scope, $rootScope, $location,
	...
	$scope.spreadVO = { "pps_skuid": "", "pps_productid": '', "pps_productid_nameref": '',"pps_ticket_limit_is":"Y","pps_product_num": 8,"pps_valid_day":30};
    ...
    $scope.getDetail = function() {
		...
		var success = function(result){
			...
			$scope.spreadList = $scope.vo.spreadList;
	...
	$scope.buildDetail = function(detailList){
		for (i in detailList) {
			...
			if ($scope.groupList[detailList[i].ppd_groupcode] === undefined ) {
				$scope.groupList.splice(detailList[i].ppd_groupcode,0, {"dataList":[detailList[i]], "num":detailList[i].ppd_groupnum});
			} else {
