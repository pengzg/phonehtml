tempApp.controller('ctr_baseCategory', function($scope,
...
$scope.editOrAdd = function(){		
    var success = function(result){
        ...
        $state.reload('index.setting.basecategory');
    }
    ...
    if ($scope.option == 2){
        ...
        data = {"bc_name":name,'bc_parent_id':$scope.selectNode.id,"bc_level":level};
    } 
    if ($scope.option == 3){
        ...
        data = {"bc_name":name,'bc_parent_id':$scope.selectNode.pId,"bc_level":level};
    } 
    if ($scope.option == 1){
        var	msg = '您确定修改本条记录吗？';
