tempApp.controller('ctr_setup_baseRole', function($scope, http, messageFactory,EzConfirm,$rootScope) {
	...
	$scope.createRole = function(){
		...
		$scope.vo.sr_roletype = '1';//所属系统 
		$scope.vo.sr_type = '1';//角色类型
		$scope.vo.sr_isgroup = 'N';//不属于集团
		var msg = "";
		if($scope.vo.sr_id){
			msg = "您确定修改本条记录吗？";
		}else{
	 ...
	 * 显示授权弹框
	 */
	$scope.showAuthDialog = function(obj){
		$scope.temp = obj;//授权的角色id
		$scope.roleAuthPop = true;
		$scope.dialogTitle = "授权";
		$scope.queryAllMenuTree(obj.sr_id,1);
	}
	...
	$scope.authIdx = function(){
		...
		$scope.enterAuth($scope.temp.sr_id,str);
	}
